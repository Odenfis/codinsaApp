...
rem ============================================================
rem  ACTUALIZADOR SISTEMA ERP NUBE (codinsaApp)
rem  Descarga el ZIP desde Google Drive y:
rem   - Copia los COD_*.exe al Escritorio del usuario
rem   - Copia los Cod*.dll a C:\Windows\SysWOW64 y los registra
rem  Requiere Windows 10 (1803+) o Windows 11.
rem
rem  PLANTILLA: los valores entre @@...@@ son reemplazados por el
rem  servidor (modulo Actualizacion ERP) al momento de la descarga.
...
set "WORKDIR=%TEMP%\codinsa_upd"
set "LOG=%TEMP%\codinsa_instalador.log"
...
if not exist "%WORKDIR%\src\COD_VENTAS.exe" goto :fail_extract
>>"%LOG%" echo [OK] Archivos extraidos

echo [4/6] Cerrando procesos abiertos del ERP...
call :KillProc COD_ALMACEN.exe
call :KillProc COD_COMPRAS.exe
call :KillProc COD_CUENTAS.exe
call :KillProc COD_SEGURIDAD.exe
call :KillProc COD_VENTAS.exe
...
call :CopyOne "%WORKDIR%\src\COD_ALMACEN.exe" "%DESKTOP%"
call :CopyOne "%WORKDIR%\src\COD_COMPRAS.exe" "%DESKTOP%"
call :CopyOne "%WORKDIR%\src\COD_CUENTAS.exe" "%DESKTOP%"
call :CopyOne "%WORKDIR%\src\COD_SEGURIDAD.exe" "%DESKTOP%"
call :CopyOne "%WORKDIR%\src\COD_VENTAS.exe" "%DESKTOP%"

call :CopyOne "%WORKDIR%\src\CodEstadisticas.dll" "%SYSTEMROOT%\SysWOW64"
call :CopyOne "%WORKDIR%\src\CodInstall.dll" "%SYSTEMROOT%\SysWOW64"
call :CopyOne "%WORKDIR%\src\CodMaestros.dll" "%SYSTEMROOT%\SysWOW64"
call :CopyOne "%WORKDIR%\src\CodProcesos.dll" "%SYSTEMROOT%\SysWOW64"
call :CopyOne "%WORKDIR%\src\CodSeguridad.dll" "%SYSTEMROOT%\SysWOW64"

echo [6/6] Registrando librerias DLL...
call :RegOne CodEstadisticas.dll
call :RegOne CodInstall.dll
call :RegOne CodMaestros.dll
call :RegOne CodProcesos.dll
call :RegOne CodSeguridad.dll
